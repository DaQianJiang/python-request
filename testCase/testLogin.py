...
class testLogin(unittest.TestCase):
    ...
    def checkResult(self,res_info):
        ...
        if (self.response.status_code==100 and self.info['code']==json.loads(res_info)['code']):
            ...
            self.result = 'success'
            dataReader().write_excel("user.xls",'login',int(self.ids),(max_cols-2),self.response.text)
            dataReader().write_excel("user.xls",'login',int(self.ids),(max_cols-1),self.result)
            print("用例通过")
        else:
            self.trueResDate = self.info
            self.result = "failed"
            ...
            dataReader().write_excel("user.xls",'login',int(self.ids),(max_cols-1),self.result)
            print("用例失败")
        self.assertTrue(self.response.status_code==100 and self.info['code']==json.loads(res_info)['code'])
